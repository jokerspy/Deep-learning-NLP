...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        ### YOUR CODE HERE
        cfg = self.config
        self.input_placeholder = tf.placeholder(tf.float32, shape=[cfg.batch_size, cfg.n_features], name="input")
        self.labels_placeholder = tf.placeholder(tf.int32, shape=[cfg.batch_size, cfg.n_classes], name="label")
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        def bias(shape,name=None):
            ...
            return tf.Variable(initial, name=name)
        cfg = self.config
        W = weight([cfg.n_features, cfg.n_classes],'weight')
        b = bias([cfg.batch_size, cfg.n_classes],'bias')
        with tf.name_scope('hidden'):
            h = tf.matmul(self.input_placeholder,W)+b
        with tf.name_scope('prediction'):
            pred = softmax(h)
        #pred = tf.to_float(self.labels_placeholder)
        ### END YOUR CODE
        return pred
