...
def softmax(x):
    ...
    with tf.name_scope('softmax'):
        c = tf.reduce_max(x,axis=1,keep_dims=True,name='offset')
        x -= c
        ex = tf.exp(x,name='exp')
        out = ex / tf.reduce_sum(ex, axis=1, keep_dims=True, name='denominator')
        #ex -= tf.expand_dims(tf.reduce_max(ex,axis=1),axis=1,name='robust_exp')
        #out = ex / tf.expand_dims(tf.reduce_sum(ex,axis=1),axis=1,name='softmax')
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    with tf.name_scope('cross_entropy_loss'):
        lgy = tf.log(yhat,name='log_y')
        out = -tf.reduce_sum(tf.to_float(y)*lgy,name='cross_entropy')
    ### END YOUR CODE
    return out
...
def test_softmax_basic():
    ...
    test1 = softmax(tf.constant(np.array([[1001, 1002], [3, 4], [3, 4]]), dtype=tf.float32))
