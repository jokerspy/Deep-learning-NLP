...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.dropout_placeholder = tf.placeholder(tf.float32, name='dropout_rate')
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        initial = tf.Variable(self.pretrained_embeddings, name='wordVec')
        lookup = tf.nn.embedding_lookup(initial, self.input_placeholder, name='lookup')
        embeddings = tf.reshape(lookup, [-1,self.config.n_features*self.config.embed_size], name='embeddings')
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, pred):
        ...
        lg = np.log(softmax)
        loss = np.sum(-self.labels_placeholder*lg)'''
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=self.labels_placeholder,name='CE'),name='mCE') 
        ### END YOUR CODE
        return loss
