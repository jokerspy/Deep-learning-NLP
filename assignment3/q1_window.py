...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for pos in range(len(sentence)):
            window = sentence[max(pos-window_size,0):pos+window_size+1]
            if pos<window_size:
                window = [start]*(window_size-pos)+window			
            if pos>len(sentence)-window_size-1:
                window += [end]*(len(sentence)-pos)
            assert len(window) == (2*window_size+1)
            windowed_data.append(((sum(window,[]), labels[pos])))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        dict = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.reshape(tf.nn.embedding_lookup(params=dict, ids=self.input_placeholder),[-1,self.config.n_window_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W',shape=[cfg.n_window_features*cfg.embed_size,cfg.hidden_size],dtype=tf.float32,initializer=initializer)
        b1 = tf.get_variable('b1',shape=cfg.hidden_size,dtype=tf.float32,initializer=initializer)
        U = tf.get_variable('U',shape=[cfg.hidden_size,cfg.n_classes],dtype=tf.float32,initializer=initializer)
        b2 = tf.get_variable('b2',shape=cfg.n_classes,dtype=tf.float32,initializer=initializer)
