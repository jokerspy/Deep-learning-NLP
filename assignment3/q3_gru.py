...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        output, preds = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32)#, self.config.max_length)
    ...
    def add_training_op(self, loss):
        ...
        if self.config.clip_gradients:
            grad, g = tf.clip_by_global_norm(grad, self.config.max_grad_norm)
            grad_and_var = [(grad[i], x[1]) for i,x in enumerate(grad_and_var)]
